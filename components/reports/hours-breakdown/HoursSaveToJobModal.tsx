"use client";
// components/reports/hours-breakdown/HoursSaveToJobModal.tsx
//
// Identical UI to condition/SaveToJobModal — just uses save-hours-report
// endpoint and takes HoursBreakdownData instead of ConditionReportData.

import React, { useState, useEffect, useRef, useCallback } from "react";
import styles from "../condition/SaveToJobModal.module.css";
import Button from "@/components/ui/Button";
import type { HoursBreakdownData } from "@/lib/reports/hours.types";

interface HoursSaveToJobModalProps {
  jobId: string;
  jobNo: string;
  companyId?: number;
...
export default function HoursSaveToJobModal({
  jobId,
  jobNo,
  companyId = 0,
  report,
  onClose,
  onSuccess,
}: HoursSaveToJobModalProps) {
  const defaultName = `Hours Breakdown ${jobNo}`;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/simpro/jobs/${jobId}/save-hours-report`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ filename: trimmed, report, companyId }),
      });
        ...
        {!isSuccess && (
          <div className={styles.body}>
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor="hours-report-filename">
                File name
              </label>
              <div className={styles.inputWrap}>
                <input
                  id="hours-report-filename"
                  ref={inputRef}
                  type="text"
                  ...
                  disabled={isSaving}
                  placeholder={`Hours Breakdown ${jobNo}`}
                  maxLength={200}
                  spellCheck={false}
                  autoComplete="off"
                />
